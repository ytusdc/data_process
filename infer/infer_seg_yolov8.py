...
def get_filepath_ls(data_dir, suffix=None):
    ...
    if Path(data_dir).is_file() and Path(data_dir.lower()).suffix in suffix:
        # data_dir 本身是一个图片文件
        return [data_dir]
...
def begin_img_infer(model_path, img_dir, save_dir, id_class_dict=None):
    ...
    for img_path in tqdm(sorted(img_path_ls)):
        ...
        cv2.resizeWindow("out_det", 800, 600)
        cv2.imshow('out_det', result_img)
        cv2.waitKey(0)
...
def main():
    ...
    img_dir = "/home/ytusdc/test_3.jpg"
    save_dir = "/home/ytusdc/测试数据/coal_result_video"

    # begin_video_infer(model_path, video_path, save_dir, id_class_dict=id_class_dict, frame_interval=1)
    begin_img_infer(model_path, img_dir, save_dir, id_class_dict=None)
