...
class VideoExtract:
    def __init__(self):
        self.interval = 25  # 默认间隔帧为25
        pass

    def get_video_attribute(self, video_cap):
        # 获取视频属性
        ...
        print(f"Width: {width}, Height: {height}，FPS: {fps}, 总帧数： {frame_count}")
        return width, height, fps

    def traverse_folder(self, folder_path, suffix=None):
        if suffix is None:
            suffix = (".mp4", ".avi")
    ...
    def clip_video_segment(self, video_path, clip_video_path, start_time, end_time):
        """
        视频截取某一段，再保存为视频
        Args:
        ...
        if not cap.isOpened():
            ...
            return
        # 获取视频属性
        width, height, fps = self.get_video_attribute(cap)
    ...
    def extract_frame(self, video_path_dir, save_dir=None, interval=25, prefix=None):
        '''
        视频抽帧
        Args:
            video_path_dir:  视频video路径, 或者视频video文件夹
            save_dir:    抽帧图片保存路径, 如果为None，则保存在视频对应目录下
        ...
        video_path_ls = self.traverse_folder(video_path_dir)
        count_frame = 0
        for video_path in video_path_ls:
            # 得到frame存储路径
            video_name = Path(video_path).stem
            if save_dir is None:
                video_parent_path = Path(video_path).parent
                save_dir = Path(video_parent_path, video_name)
            else:
...
def video_clip_segment():
    ...
    video_extract_cls =  VideoExtract()
    video_extract_cls.clip_video_segment(input_video_path, output_video_path, start_time, end_time)

def video_extract():
    video_path = "/home/ytusdc/测试数据/车辆/192.168.10.123/00000000486000000.mp4"
    save_dir  = "/home/ytusdc/测试数据/车辆/192.168.10.123/extract_frame_2"
    interval = 60

    video_extract_cls =  VideoExtract()
    video_extract_cls.extract_frame(video_path, save_dir=save_dir, interval=interval)
